...
def _register_pdf_font() -> str:
    ...
    # TODO: Bundle or install a proper Persian-capable font for consistent
    # Streamlit Cloud rendering. Helvetica is a safe PDF fallback, but Persian
    # glyph support and shaping will be limited without a real Persian font.
...
def _metric_table(metrics: list[tuple[str, str]], styles: dict) -> Table:
    rows = [
        [_paragraph(label, styles["body"]), _paragraph(value, styles["body"])]
...
def _recent_records_table(recent_records, styles: dict) -> Table:
    ...
    rows = [[_paragraph(header, styles["body"]) for header in headers]]

    for _, record in recent_records.iterrows():
        rows.append(
            [
                _paragraph(WEEKDAY_LABELS.get(record["day"], record["day"]), styles["body"]),
                ...
                _paragraph(record["status"], styles["body"]),
            ]
        )
...
def generate_pdf_report(
    ...
    story = [
        _paragraph("گزارش روزانه نرخ دلار بانک ملی", styles["title"]),
        _paragraph(f"امروز: {_format_jalali_date(latest_record['date'])}", styles["subtitle"]),
        Spacer(1, 5 * mm),
        _paragraph("شاخص‌های کلیدی", styles["section"]),
        _metric_table(
            [
        ...
        Spacer(1, 7 * mm),
        _paragraph(recommendation["title"], styles["section"]),
        _paragraph(recommendation["headline"], styles["headline"]),
        _paragraph(recommendation["message"], styles["body"]),
        Spacer(1, 4 * mm),
        _metric_table(
            [
                ("اختلاف امروز", _format_percent(latest_record["difference_percent"])),
        ...
        PageBreak(),
        _paragraph("۷ رکورد اخیر", styles["title"]),
        _paragraph("مرتب‌شده بر اساس تاریخ گزارش برای مقایسه سریع", styles["subtitle"]),
        Spacer(1, 5 * mm),
        _paragraph("خلاصه آخرین وضعیت", styles["section"]),
        _metric_table(
            [
